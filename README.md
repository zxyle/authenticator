...
- **一键导入 Google Authenticator**：
  1. 在手机 Google Authenticator 中点击右上角菜单 →「转移账号」→「导出账号」；
  2. 勾选要导出的账号，显示二维码后，用另一台设备拍下二维码（截图通常会被系统限制）；
  3. 打开扩展「选项」页，在「一键导入 Google Authenticator」中上传二维码图片；
  4. 可选填写「批量设置域名匹配」，点击「预览导入」确认后点击「确认导入」；
  5. 导入完成后会显示新增/跳过数量；重复导入同一批账号会自动去重。
